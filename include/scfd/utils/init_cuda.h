...
inline int init_cuda( int log_lev, Log &log, int pci_id, int dev_num = -2 )
{
    ...
    if ( count > 1 )
    {
        if ( ( pci_id == -1 ) || ( dev_num == -1 ) )
        {
            for ( i = 0; i < count; i++ )
            {
                ...
                SCFD_CUDA_SAFE_CALL( cudaGetDeviceProperties( &device_prop, i ) );
                //printf( "#%i:   %s, pci-bus id:%i %i %i \n", i, (char*)&device_prop,device_prop.pciBusID,device_prop.pciDeviceID,device_prop.pciDomainID);
                log.info_f(
                    log_lev, "init_cuda: #%i:   %s, pci-bus id:%i %i %i ", i, (char *)&device_prop,
                    device_prop.pciBusID, device_prop.pciDeviceID, device_prop.pciDomainID
                );
            }
        ...
        else if ( pci_id == -2 )
        {
            res_dev_num = dev_num;
            cudaDeviceProp device_prop;
            SCFD_CUDA_SAFE_CALL( cudaGetDeviceProperties( &device_prop, res_dev_num ) );

            //printf("Using #%i:   %s@[%i:%i:%i]\n",res_dev_num,(char*)&device_prop,device_prop.pciBusID,device_prop.pciDeviceID,device_prop.pciDomainID);
            log.info_f(
                log_lev, "init_cuda: Using #%i:   %s@[%i:%i:%i]", res_dev_num, (char *)&device_prop,
                device_prop.pciBusID, device_prop.pciDeviceID, device_prop.pciDomainID
            );
        }
        else if ( dev_num == -2 )
        {
            ...
            {
                SCFD_CUDA_SAFE_CALL( cudaGetDeviceProperties( &device_prop, j ) );
            ...
            //printf("Using #%i:   %s@[%i:%i:%i]\n", res_dev_num,(char*)&device_prop,device_prop.pciBusID,device_prop.pciDeviceID,device_prop.pciDomainID);
            log.info_f(
                log_lev, "Using #%i:   %s@[%i:%i:%i]", res_dev_num, (char *)&device_prop, device_prop.pciBusID,
                device_prop.pciDeviceID, device_prop.pciDomainID
            );
        }
        else
        {
            throw std::runtime_error(
                "init_cuda: Incorrect arguments: pci_id == " + std::to_string( pci_id ) +
    ...
    else
    {
        ...
        printf( "       using it...\n");*/
        log.info_f(
            log_lev, "init_cuda: There is only one compartable CUDA device. It will be used regardless of preference"
        );
        log.info_f(
            log_lev, "init_cuda: #%i:   %s, pci-bus id:%i %i %i ", res_dev_num, (char *)&device_prop,
            device_prop.pciBusID, device_prop.pciDeviceID, device_prop.pciDomainID
        );
        log.info_f( log_lev, "init_cuda:        using it..." );
    }

    SCFD_CUDA_SAFE_CALL( cudaSetDevice( res_dev_num ) );
...
inline int init_cuda_persistent( Log &log, std::size_t device_memory_in_MB, std::size_t max_minutes = 10 )
{
    ...
    for ( int dev = 0; dev < count; dev++ )
    {
        ...
        SCFD_CUDA_SAFE_CALL( cudaSetDevice( dev ) );
        ...
        SCFD_CUDA_SAFE_CALL( cudaDeviceReset() );
    }
    if ( device_memory_in_MB > 0 )
    {
        ...
        {
            throw std::runtime_error(
                "init_cuda_persistent: no suitable device found with minimum required memory.\n  Maximum found device "
                "memory is " +
                std::to_string( save_coefficient * max_total_mem / 1000.0 / 1000.0 ) + "MB \n"
            );
        }
        while ( true )
        {
    ...
    {
        std::sort(
            device_mems.begin(), device_mems.end(),
    ...
    {
        throw std::runtime_error(
            "init_cuda_persistent: failed to find a suitable device for the given time interval.\n"
        );
    }
    cudaDeviceProp device_prop;
    SCFD_CUDA_SAFE_CALL( cudaGetDeviceProperties( &device_prop, res_dev_num ) );
    log.info_f(
        log_lev, "init_cuda_persistent%s: Using #%i:   %s@[%i:%i:%i], %i MB",
        device_memory_in_MB == 0 ? "_best_mem" : " ", res_dev_num, (char *)&device_prop, device_prop.pciBusID,
        device_prop.pciDeviceID, device_prop.pciDomainID, std::size_t( device_prop.totalGlobalMem / 1000.0 / 1000.0 )
    );
