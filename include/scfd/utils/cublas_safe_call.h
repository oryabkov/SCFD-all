...
static const char *_cublasGetErrorEnum( cublasStatus_t error )
{
    ...
    case CUBLAS_STATUS_NOT_INITIALIZED:
        return "The cuBLAS library was not initialized. This is usually caused by the lack of a prior cublasCreate() "
               "call, an error in the CUDA Runtime API called by the cuBLAS routine, or an error in the hardware "
               "setup.";

    case CUBLAS_STATUS_ALLOC_FAILED:
        return "Resource allocation failed inside the cuBLAS library. This is usually caused by a cudaMalloc() "
               "failure.";
    ...
    case CUBLAS_STATUS_LICENSE_ERROR:
        return "The functionnality requested requires some license and an error was detected when trying to check the "
               "current licensing. This error can happen if the license is not present or is expired or if the "
