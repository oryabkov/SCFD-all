...
inline int init_hip( int log_lev, Log &log, int pci_id, int dev_num = -2 )
{
    ...
    if ( count > 1 )
    {
        ...
        else if ( pci_id == -2 )
        {
            ...
            log.info_f(
                log_lev, "init_hip: Using #%i:   %s@[%i:%i:%i]", res_dev_num, (char *)&device_prop,
        ...
        else
        {
            throw std::runtime_error(
                "init_hip: Incorrect arguments: pci_id == " + std::to_string( pci_id ) +
    ...
    else
    {
        ...
        printf( "       using it...\n");*/
        log.info_f(
            log_lev, "init_hip: There is only one compartable HIP device. It will be used regardless of preference"
        );
        log.info_f(
            log_lev, "init_hip: #%i:   %s, pci-bus id:%i %i %i ", res_dev_num, (char *)&device_prop,
            device_prop.pciBusID, device_prop.pciDeviceID, device_prop.pciDomainID
        );
...
inline int init_hip_persistent( Log &log, std::size_t device_memory_in_MB, std::size_t max_minutes = 10 )
{
    ...
    if ( device_memory_in_MB > 0 )
    {
        ...
        {
            throw std::runtime_error(
                "init_hip_persistent: no suitable device found with minimum required memory.\n  Maximum found device "
                "memory is " +
