...
#ifndef __SCFD_UTILS_INITHIPMPI_H__
#define __SCFD_UTILS_INITHIPMPI_H__
...
inline int init_hip_mpi(Log &log, const scfd::communication::mpi_comm_info& comm, int shift_index = 0)
{
    ...
    log.info_f("init_hip_mpi split_color: node_name = %s, node_color = %i, global_size = %i, global_id = %i, node_size = %i, devices_on_node = %i, node_device_id = %i, node_my_id = %i", node_name, char_hash, comm.num_procs, comm.myid, node_size, number_of_devices_on_node, device_id, my_id); 
