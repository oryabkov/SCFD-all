...
void trash_cuda_memory()
{
    ...
    SCFD_CUDA_SAFE_CALL( cudaMemGetInfo( &free_device_mem, &total_device_mem ) );
    void  *all_ptr;
    size_t trash_device_mem = free_device_mem - 0x1000000;
    SCFD_CUDA_SAFE_CALL( cudaMalloc( &all_ptr, trash_device_mem ) );
    SCFD_CUDA_SAFE_CALL( cudaMemset( all_ptr, 255, trash_device_mem ) );
    SCFD_CUDA_SAFE_CALL( cudaFree( all_ptr ) );
}
