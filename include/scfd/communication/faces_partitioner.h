...
struct faces_partitioner
{
    ...
    faces_partitioner( const Communicator &comm, const HostMesh &mesh, const Map &map_elem )
        : my_rank( comm.my_rank() ), is_complete( false )
    {
        ...
        for ( Ord i = 0; i < map_elem.get_size(); ++i )
        {
            ...
            for ( Ord face_i = 0; face_i < mesh.get_elem_faces_num( elem_glob_i ); ++face_i )
            {
                ...
                if ( ranks.find( faces[face_i] ) != ranks.end() )
                    continue;
                ...
                {
                    if ( face_elems[j] < min_elem_id )
                        min_elem_id = face_elems[j];
                }
    ...
    int get_rank( Ord i_glob ) const
    {
        ...
        assert( it != ranks.end() );
        if ( it->second == -1 )
            throw std::logic_error( "faces_partitioner:: not realized yet!!" );
        return it->second;
    }
