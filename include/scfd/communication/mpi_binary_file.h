...
struct mpi_binary_file
{
    ...
    mpi_binary_file(
        const mpi_comm_info &comm_info, const std::string &file_name, int amode = MPI_MODE_RDWR | MPI_MODE_CREATE
    )
    {
    ...
    {
        SCFD_MPI_SAFE_CALL( MPI_File_write(
            fh, data, count * sizeof( T ), MPI_BYTE, detail::raw_status( static_cast<detail::mpi_status *>( nullptr ) )
        ) );
    }
    void write_at( MPI_Offset offset, const T *data, int count ) const
    {
        SCFD_MPI_SAFE_CALL( MPI_File_write_at(
            fh, offset * sizeof( T ), data, count * sizeof( T ), MPI_BYTE,
            detail::raw_status( static_cast<detail::mpi_status *>( nullptr ) )
        ) );
    }
    void read( int count, T *data ) const
    {
        SCFD_MPI_SAFE_CALL( MPI_File_read(
            fh, data, count * sizeof( T ), MPI_BYTE, detail::raw_status( static_cast<detail::mpi_status *>( nullptr ) )
        ) );
    }
    void read_at( MPI_Offset offset, int count, T *data ) const //WARNING! for some reason it doesn't work as expexted!
    {
        SCFD_MPI_SAFE_CALL( MPI_File_read_at(
            fh, offset * sizeof( T ), data, count, MPI_BYTE,
            detail::raw_status( static_cast<detail::mpi_status *>( nullptr ) )
        ) );
    }
