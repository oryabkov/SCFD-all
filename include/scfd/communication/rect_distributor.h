...
#ifndef __SCFD_RECT_DISTRIBUTOR_H__
#define __SCFD_RECT_DISTRIBUTOR_H__
...
//2.rect_partitioner should later be equipped with a CommInfo template parameter too
...
template <
    class T, arrays::ordinal_type Dim, class Memory, arrays::ordinal_type TensorDim,
    template <arrays::ordinal_type...> class Arranger>
...
template <class T, arrays::ordinal_type Dim, class Memory, template <arrays::ordinal_type...> class Arranger>
arrays::tensor1_array_nd<T, Dim, Memory, arrays::dyn_dim>
array_as_tensor1_array( const arrays::array_nd<T, Dim, Memory, Arranger> &array )
{
...
template <
    ...
    template <arrays::ordinal_type...> class Arranger>
arrays::tensor1_array_nd<T, Dim, Memory, arrays::dyn_dim>
array_as_tensor1_array( const arrays::tensor1_array_nd<T, Dim, Memory, TensorDim, Arranger> &array )
{
    arrays::tensor1_array_nd<T, Dim, Memory, arrays::dyn_dim> res;
    res.init_by_raw_data( array.raw_ptr(), array.rect_nd(), get_array_tensor_dim( array ) );
    return res;
...
struct rect_distributor
{
    ...
    void sync( const Array &array ) const
    {
        ...
        Ord isend_requests_count = 0, irecv_requests_count = 0;
...
private:
    ...
    )
    {
        ...
        {
            throw std::logic_error( "rect_distributor::init_packet: niether sender nor reciever is my_id" );
        }
