...
class tensor_array_nd_visible
    : public tensor_array_nd<T, ND, typename Memory::host_memory_type, Arranger, TensorDims...>
{
...
private:
    ...
    typename std::enable_if<has_separate_buffer_>::type init_by_( const array_type &array )
    {
...
public:
    ...
    tensor_array_nd_visible( Args... args )
    {
        init( args... );
    }
    ...
    void sync_to_array() const
    {
        if ( !has_separate_buffer )
            return;
        array_memory_type::copy_from_host(
            sizeof( T ) * arranger_type::total_size(), this->raw_ptr(), array_.raw_ptr()
        );
    }
