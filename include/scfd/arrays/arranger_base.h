...
public:
    ...
    {
        return detail::dim_getter_ < Ord, Ind, detail::template_indexer<Ord, Ind, Dims...>::value != dyn_dim,
               Dims... > ::template get<dynamic_dims_num>( dyn_dims_ );
    }
#ifdef SCFD_ARRAYS_ENABLE_INDEX_SHIFT
    template <Ord Ind>
    __DEVICE_TAG__ Ord get_index0() const
    {
        return detail::index0_getter_ < Ord, Ind, detail::template_indexer<Ord, Ind, Dims...>::value != dyn_dim,
