...
struct nested_initializer_list_dims_<T, DimsNum, CurrIdx, false>
{
    ...
    template <class DimsVec>
    static void
    calc( std::initializer_list<typename nested_initializer_list_gen<T, DimsNum - CurrIdx>::type> il, DimsVec &dims )
    {
        if ( il.size() == 0 )
        {
            throw std::logic_error(
                "nested_initializer_list_dims_::empty initializer list case is not implemented - TODO!!"
            );
        }
    ...
    static bool check_if_square(
        std::initializer_list<typename nested_initializer_list_gen<T, DimsNum - CurrIdx>::type> il, DimsVec &dims
    )
    {
        if ( dims[CurrIdx] != il.size() )
            return false;
        for ( ordinal_type i1 = 0; i1 < il.size(); ++i1 )
        {
            if ( !next_dims_t::check_if_square( *( il.begin() + i1 ), dims ) )
                return false;
        }
        return true;
    }
...
struct nested_initializer_list_dims
{
    ...
    template <class DimsVec>
    static void calc( std::initializer_list<typename nested_initializer_list_gen<T, DimsNum>::type> il, DimsVec &dims )
    {
        dims_t::calc( il, dims );
    }
    template <class DimsVec>
    static bool
    check_if_square( std::initializer_list<typename nested_initializer_list_gen<T, DimsNum>::type> il, DimsVec &dims )
    {
