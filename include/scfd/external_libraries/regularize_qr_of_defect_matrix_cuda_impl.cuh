...
template <class T>
__global__ void ker_get_matrix_diag_with_indices( int n, const T *A, thrust::pair<T, int> *diag_with_indices )
{
...
template <class T>
__global__ void
ker_fill_diag_degenerate_flags( int defect, const thrust::pair<T, int> *diag_with_indices, bool *diag_degenerate_flags )
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if ( !( i < defect ) )
        return;
...
template <class T>
__global__ void ker_r_matrix_correct_defect_part( int n, const bool *diag_degenerate_flags, T *A )
{
    ...
    if ( !( ( i < n ) && ( j < n ) ) )
        return;
    if ( i > j )
        return;   /// Lower part is not affected
...
void regularize_qr_of_defect_matrix_cuda( int sz, T *A, bool *diag_degenerate_flags, int defect )
{
    ...
    thrust::fill(
        thrust::device_pointer_cast( diag_degenerate_flags ), thrust::device_pointer_cast( diag_degenerate_flags ) + sz,
        false
    );
