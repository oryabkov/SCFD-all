...
template <class T>
__global__ void ker_r_inv_matrix_set_ident( int n, T *r_inv_mat )
{
...
template <class T>
__global__ void ker_precalc_invert_r_diag( int n, T *mat_tmp, T *r_diag_inv )
{
...
template <class T>
__global__ void ker_invert_r_diag( int n, const T *r_diag_inv, T *mat_tmp, T *r_inv_mat )
{
    int ind = blockIdx.x * blockDim.x + threadIdx.x;
    if ( !( ind < n * n ) )
        return;
...
template <class T>
__global__ void ker_copy_mat_tmp_col_i( int n, int i, const T *mat_tmp, T *mat_tmp_col_i )
{
...
template <class T>
__global__ void ker_back_elimination( int n, int i, const T *mat_tmp_col_i, T *r_inv_mat )
{
    int ind = blockIdx.x * blockDim.x + threadIdx.x;
    if ( !( ind < ( n - i ) * i ) )
        return;
    int i1 = ind % i, j = i + ind / i;
...
void invert_upper_triangular_dense_matrix_cuda( int sz, T *r_mat, T *r_diag_inv, T *mat_tmp_col_i, T *r_inv_mat )
{
    ...
    for ( int i = sz - 1; i >= 0; --i )
    {
        ...
        detail::ker_back_elimination<<<( ( ( sz - i ) * i ) / block_size ) + 1, block_size>>>(
            sz, i, mat_tmp_col_i, r_inv_mat
        );
