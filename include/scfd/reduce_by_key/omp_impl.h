...
#ifndef __SCFD_REDUCE_BY_KEY_OMP_IMPL_H__
#define __SCFD_REDUCE_BY_KEY_OMP_IMPL_H__
...
template <class Ord>
Ord omp_reduce_by_key_chunk_begin( Ord size, int tid, int threads )
{
...
#pragma omp parallel
    {
        ...
        const Ord chunk_beg = detail::omp_reduce_by_key_chunk_begin( size, tid, nthreads );
        const Ord chunk_end = detail::omp_reduce_by_key_chunk_begin( size, tid + 1, nthreads );
...
#pragma omp parallel num_threads( threads )
    {
        const int tid       = omp_get_thread_num();
        const Ord chunk_beg = detail::omp_reduce_by_key_chunk_begin( size, tid, threads );
        const Ord chunk_end = detail::omp_reduce_by_key_chunk_begin( size, tid + 1, threads );
        ...
        for ( Ord i = chunk_beg; i < chunk_end; ++i )
        {
            ...
            Value     val = values_in[i];
            Ord       j   = i + 1;
            while ( j < size && key_equal( key, keys_in[j] ) )
            {
                val = binary_op( val, values_in[j] );
                ++j;
            }
