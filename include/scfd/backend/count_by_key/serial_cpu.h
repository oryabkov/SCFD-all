...
#ifndef __SCFD_SERIAL_CPU_COUNT_BY_KEY_H__
#define __SCFD_SERIAL_CPU_COUNT_BY_KEY_H__
...
template <class Ord, class Key, class Count, class KeyEqual>
Ord count_by_key_host_impl( Ord size, const Key *keys_in, Key *keys_out, Count *counts_out, KeyEqual key_equal )
{
    if ( size <= 0 )
        return 0;
    ...
    for ( Ord i = 1; i < size; ++i )
    {
        ...
        else
        {
            ...
            counts_out[out_size] = current_cnt;
            ++out_size;
...
struct serial_cpu_count_by_key
{
    ...
    {
        return detail::count_by_key_host_impl( size, keys_in, keys_out, counts_out, key_equal );
    }
