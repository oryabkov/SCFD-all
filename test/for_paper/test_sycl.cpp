...
template <>
struct sycl::is_device_copyable<func_cross_prod<array_device_t>> : std::true_type
{
};
...
#pragma omp parallel for
    ...
    std::vector<double> dev_tensor;
    dev_tensor.reserve( number_of_iters );
    for ( int it_ = 0; it_ < number_of_iters; it_++ )
    {
    ...
    std::cout << "device tensor time = "
              << std::accumulate( begin( dev_tensor ), end( dev_tensor ), 0.0 ) / number_of_iters << "ms." << std::endl;
