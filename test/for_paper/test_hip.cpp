...
int main(int argc, char const *argv[])
{
    ...
    if((tests == 'd')||(tests == 'a'))
    {
        ...
        HIP_SAFE_CALL( hipMemcpy( (void*) cross_ptr_host_check, (void*)cross_ptr_dev, sizeof(T)*total_size, hipMemcpyDeviceToHost ) );
        std::cout << "gpu ptr diff    = " << check_coincide_ptr(N, cross_ptr_host, cross_ptr_host_check) << std::endl;
        HIP_SAFE_CALL( hipMemcpy( (void*) cross_ptr_ok_host_check, (void*)cross_ptr_ok_dev, sizeof(T)*total_size, hipMemcpyDeviceToHost ) );
