...
struct func_cross_prod
{
    ...
    func_cross_prod( const Vec3 &f1, const Vec3 &f2, Vec3 &f_out ) : f1_( f1 ), f2_( f2 ), f_out_( f_out )
    {
    }
...
int main( int argc, char const *argv[] )
{
    ...
    if ( argc != 4 )
    {
        ...
        std::cout << "where: N is a size of R^{3XN}, iters is number of iterations for better measurements,"
                  << std::endl;
...
#pragma omp parallel for
    for ( std::size_t j = 0; j < N; j++ )
    {
        ...
        cross_ptr_ok_host[IG( j, 0 )] = u_ptr_ok_host[IG( j, 1 )] * v_ptr_ok_host[IG( j, 2 )] -
                                        u_ptr_ok_host[IG( j, 2 )] * v_ptr_ok_host[IG( j, 1 )];
        cross_ptr_ok_host[IG( j, 1 )] =
            -( u_ptr_ok_host[IG( j, 0 )] * v_ptr_ok_host[IG( j, 2 )] -
               u_ptr_ok_host[IG( j, 2 )] * v_ptr_ok_host[IG( j, 0 )] );
    ...
    if ( ( tests == 'd' ) || ( tests == 'a' ) )
    {
        ...
        {
            cross_prod_device<for_each_hip_t, array_device_t>( N, u_dev, v_dev, cross_dev );
        }

        std::vector<double> gpu_tensor;
