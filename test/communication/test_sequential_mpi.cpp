...
int main( int argc, char *argv[] )
{
    ...
    if ( mpi_comm.myid == 0 )
    {
        std::cout << "the following output should be in sequential order of proc = ..., and proc_running = ..."
                  << std::endl;
    }
    seq_mpi.start();
    ...
    seq_mpi.stop();
    std::cout << "after_barrier: "
              << "proc = " << mpi_comm.myid << ", proc_running = " << proc_running << std::endl;
