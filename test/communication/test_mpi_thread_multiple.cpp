...
int main( int argc, char *argv[] )
{
    ...
    if ( ( comm_info.myid == 0 ) && ( thread == -1 ) )
    {
        ...
        std::cout << " thread=1: MPI_THREAD_FUNNELED: Indicating that if the process is multithreaded, only the thread "
                     "that called MPI_Init_thread will make MPI calls."
                  << std::endl;
        std::cout << " thread=2: MPI_THREAD_SERIALIZED: Indicating that if the process is multithreaded, only one "
                     "thread will make MPI library calls at one time."
                  << std::endl;
        std::cout << " thread=3: MPI_THREAD_MULTIPLE: Indicating that if the process is multithreaded, multiple "
