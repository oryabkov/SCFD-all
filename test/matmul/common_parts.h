...
if ( argc != 4 )
{
    ...
    std::cout << "where: N is a size of R^{K^2 * N}, iters is number of iterations for better measurements, K is a "
                 "static parameter."
              << std::endl;
...
T   values_range = 100, error_mlups = 50, error_mul = values_range * error_mlups;
int errors_num = 0;
...
if ( ( tests == 'd' ) || ( tests == 'a' ) )
{
    std::cout << "executing device tests ... " << std::endl;
    {
        ...
        device_e2.record();
        std::cout << "device tensor naive time = " << device_e2.elapsed_time( device_e1 ) / number_of_iters << "ms."
                  << std::endl;
        if ( tests == 'a' )
        {
    ...
    }
    {
        ...
        device_e2.record();
        std::cout << "device tensor mm time = " << device_e2.elapsed_time( device_e1 ) / number_of_iters << "ms."
                  << std::endl;
        if ( tests == 'a' )
        {
            mat_mul_dev_view.init( mat_mul_dev, true );
    ...
    }
    {
        ...
        device_e2.record();
        std::cout << "device tensor time = " << device_e2.elapsed_time( device_e1 ) / number_of_iters << "ms."
                  << std::endl;
        if ( tests == 'a' )
        {
            mat_mul_dev_view.init( mat_mul_dev, true );
...
#endif
    {
        ...
        __COMMON_PARTS_SAFE_CALL__( __COMMON_PARTS_DEVICE_MEMCPY__(
            (void *)u_ptr_func_dev, (void *)u_ptr_host, sizeof( T ) * total_size,
            __COMMON_PARTS_DEVICE_MEMCPY_HOST_TO_DEVICE__
        ) );
        __COMMON_PARTS_SAFE_CALL__( __COMMON_PARTS_DEVICE_MEMCPY__(
            (void *)v_ptr_func_dev, (void *)v_ptr_host, sizeof( T ) * total_size,
            __COMMON_PARTS_DEVICE_MEMCPY_HOST_TO_DEVICE__
        ) );
        ...
        device_e2.record();
        std::cout << "device ptr_func naive time = " << device_e2.elapsed_time( device_e1 ) / number_of_iters << "ms."
                  << std::endl;

        if ( tests == 'a' )
        {
            __COMMON_PARTS_SAFE_CALL__( __COMMON_PARTS_DEVICE_MEMCPY__(
                (void *)mat_mul_ptr_func_check, (void *)mat_mul_ptr_func_dev, sizeof( T ) * total_size,
                __COMMON_PARTS_DEVICE_MEMCPY_DEVICE_TO_HOST__
            ) );
    ...
    /***********************************************************************************************************/
    {
        ...
        __COMMON_PARTS_SAFE_CALL__( __COMMON_PARTS_DEVICE_MEMCPY__(
            (void *)u_ptr_func_dev, (void *)u_ptr_ok_host, sizeof( T ) * total_size,
            __COMMON_PARTS_DEVICE_MEMCPY_HOST_TO_DEVICE__
        ) );
        __COMMON_PARTS_SAFE_CALL__( __COMMON_PARTS_DEVICE_MEMCPY__(
            (void *)v_ptr_func_dev, (void *)v_ptr_ok_host, sizeof( T ) * total_size,
            __COMMON_PARTS_DEVICE_MEMCPY_HOST_TO_DEVICE__
        ) );
#else
        ...
        for ( int it_ = 0; it_ < number_of_iters; it_++ )
        {
            ...
            std::chrono::duration<double, std::milli> elapsed_seconds = end - start;
            if ( it_ > 0 )
                gpu_ptr_func_dev.push_back( elapsed_seconds.count() );
        }

        device_e2.record();
        std::cout << "device ptr_func_dev time = " << device_e2.elapsed_time( device_e1 ) / number_of_iters << "ms."
                  << std::endl;
    ...
    /***********************************************************************************************************/
    {
        ...
        for ( int it_ = 0; it_ < number_of_iters; it_++ )
        {
            ...
            std::chrono::duration<double, std::milli> elapsed_seconds = end - start;
            if ( it_ > 0 )
                cpu_ptr_func_dev.push_back( elapsed_seconds.count() );
        }

        device_e2.record();
        std::cout << "device ptr_func_host time = " << device_e2.elapsed_time( device_e1 ) / number_of_iters << "ms."
                  << std::endl;
...
#ifndef __COMMON_PARTS_USING_SYCL__
    {
        ...
        __COMMON_PARTS_SAFE_CALL__(
            __COMMON_PARTS_DEVICE_MALLOC__( (void **)&mat_mul_ptr_dev, sizeof( T ) * total_size )
        );

        __COMMON_PARTS_SAFE_CALL__( __COMMON_PARTS_DEVICE_MEMCPY__(
            (void *)u_ptr_dev, (void *)u_ptr_host, sizeof( T ) * total_size,
            __COMMON_PARTS_DEVICE_MEMCPY_HOST_TO_DEVICE__
        ) );
        __COMMON_PARTS_SAFE_CALL__( __COMMON_PARTS_DEVICE_MEMCPY__(
            (void *)v_ptr_dev, (void *)v_ptr_host, sizeof( T ) * total_size,
            __COMMON_PARTS_DEVICE_MEMCPY_HOST_TO_DEVICE__
        ) );
        ...
        device_e2.record();
#    ifdef USE_CONST
        std::cout << "device ptr_const time = " << device_e2.elapsed_time( device_e1 ) / number_of_iters << "ms."
                  << std::endl;
#    else
        std::cout << "device ptr time = " << device_e2.elapsed_time( device_e1 ) / number_of_iters << "ms."
                  << std::endl;
#    endif
        if ( tests == 'a' )
        {
            __COMMON_PARTS_SAFE_CALL__( __COMMON_PARTS_DEVICE_MEMCPY__(
                (void *)mat_mul_ptr_check, (void *)mat_mul_ptr_dev, sizeof( T ) * total_size,
                __COMMON_PARTS_DEVICE_MEMCPY_DEVICE_TO_HOST__
            ) );
...
#    endif
    ...
    /***************************************************************************************************************/
    {
        ...
        __COMMON_PARTS_SAFE_CALL__( __COMMON_PARTS_DEVICE_MEMCPY__(
            (void *)u_ptr_ok_dev, (void *)u_ptr_ok_host, sizeof( T ) * total_size,
            __COMMON_PARTS_DEVICE_MEMCPY_HOST_TO_DEVICE__
        ) );
        __COMMON_PARTS_SAFE_CALL__( __COMMON_PARTS_DEVICE_MEMCPY__(
            (void *)v_ptr_ok_dev, (void *)v_ptr_ok_host, sizeof( T ) * total_size,
            __COMMON_PARTS_DEVICE_MEMCPY_HOST_TO_DEVICE__
        ) );
        ...
        device_e2.record();
#    ifdef USE_CONST
        std::cout << "device ptr_ok_const time = " << device_e2.elapsed_time( device_e1 ) / number_of_iters << "ms."
                  << std::endl;
#    else
        std::cout << "device ptr_ok time = " << device_e2.elapsed_time( device_e1 ) / number_of_iters << "ms."
                  << std::endl;
#    endif
        if ( tests == 'a' )
        {
            __COMMON_PARTS_SAFE_CALL__( __COMMON_PARTS_DEVICE_MEMCPY__(
                (void *)mat_mul_ptr_ok_check, (void *)mat_mul_ptr_ok_dev, sizeof( T ) * total_size,
                __COMMON_PARTS_DEVICE_MEMCPY_DEVICE_TO_HOST__
            ) );
...
#    endif
    ...
    }
    {
        ...
        device_e2.record();
#    ifdef USE_CONST
        std::cout << "device ptr_ok_mm_const time = " << device_e2.elapsed_time( device_e1 ) / number_of_iters << "ms."
                  << std::endl;
#    else
        std::cout << "device ptr_ok_mm time       = " << device_e2.elapsed_time( device_e1 ) / number_of_iters << "ms."
                  << std::endl;
#    endif
        if ( tests == 'a' )
        {
...
#    endif
    ...
    {
        std::string filename;
        filename = "mat_mul_" + target_name + "_" + file_type + "_" + std::to_string( N ) + "_dev_tensor_ptr_" +
                   std::to_string( K ) + "x" + std::to_string( K ) + ".csv";
    ...
    {
        std::string filename;
        filename = "mat_mul_" + target_name + "_" + file_type + "_" + std::to_string( N ) + "_dev_ptr_arrange_" +
                   std::to_string( K ) + "x" + std::to_string( K ) + ".csv";
        std::fstream out_file{ filename, out_file.out };
        if ( !out_file.is_open() )
            std::cout << "failed to open " << filename << '\n';
        else
        {
            ...
            for ( int j = 0; j < number_of_iters - 1; j++ )
            {
                out_file << cpu_ptr_func_dev.at( j ) << "," << gpu_ptr_func_dev.at( j ) << "," << gpu_tensor.at( j )
                         << std::endl;
            }
            out_file.close();
        }
    ...
    {
        std::string filename;
        filename = "mat_mul_" + target_name + "_" + file_type + "_" + std::to_string( N ) + "_dev_cmp_tensors_" +
                   std::to_string( K ) + "x" + std::to_string( K ) + ".csv";
        std::fstream out_file{ filename, out_file.out };
        if ( !out_file.is_open() )
            std::cout << "failed to open " << filename << '\n';
        else
        {
            ...
            for ( int j = 0; j < number_of_iters - 1; j++ )
            {
                out_file << gpu_tensor_naive.at( j ) << "," << gpu_tensor.at( j ) << "," << gpu_tensor_mm.at( j )
                         << std::endl;
            }
            out_file.close();
        }
...
if ( ( tests == 'h' ) || ( tests == 'a' ) )
{
    ...
    std::vector<double> host_tensor;
    host_tensor.reserve( number_of_iters );
    std::vector<double> host_ptr;
    host_ptr.reserve( number_of_iters );
    std::vector<double> host_ptr_ok;
    host_ptr_ok.reserve( number_of_iters );
        ...
        for ( std::size_t n = 0; n < N; ++n )
        {
            for ( std::size_t i = 0u; i < K; ++i )
                for ( std::size_t j = 0u; j < K; ++j )
                {
                    ...
                    for ( std::size_t k = 0u; k < K; ++k )
                        mat_mul_ptr_ok_host[IG( n, i, j )] +=
                            u_ptr_ok_host[IG( n, i, k )] * v_ptr_ok_host[IG( n, k, j )];
                }
        }
        auto                          end             = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double> elapsed_seconds = end - start;
...
#pragma omp parallel for
    ...
    {
        std::string filename;
        filename = "mat_mul_" + file_type + "_" + std::to_string( N ) + "_device_host_" + std::to_string( K ) + "x" +
                   std::to_string( K ) + ".csv";
        ...
        else
        {
            ...
            for ( int j = 0; j < number_of_iters - 1; j++ )
            {
                out_file_cpu << host_tensor.at( j ) << "," << host_ptr.at( j ) << "," << host_ptr_ok.at( j )
                             << std::endl;
            }
            out_file_cpu.close();
        }
