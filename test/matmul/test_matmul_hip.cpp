...
using for_each_hip_t = scfd::for_each::hip<>;
...
int main(int argc, char const *argv[])
{
    ...
    HIP_SAFE_CALL( hipMalloc( (void**)&mat_mul_ptr_ok_dev , sizeof(T)*total_size ) );

    HIP_SAFE_CALL( hipMalloc( (void**)&u_ptr_dev        , sizeof(T)*total_size ) );
    HIP_SAFE_CALL( hipMalloc( (void**)&v_ptr_dev        , sizeof(T)*total_size ) );
    HIP_SAFE_CALL( hipMalloc( (void**)&mat_mul_ptr_dev  , sizeof(T)*total_size ) );

    HIP_SAFE_CALL( hipMalloc( (void**)&u_ptr_ok_dev       , sizeof(T)*total_size ) );
    HIP_SAFE_CALL( hipMalloc( (void**)&v_ptr_ok_dev       , sizeof(T)*total_size ) );
    HIP_SAFE_CALL( hipMalloc( (void**)&mat_mul_ptr_ok_dev , sizeof(T)*total_size ) );

    HIP_SAFE_CALL( hipMalloc( (void**)&u_ptr_func_dev       , sizeof(T)*total_size ) );
    HIP_SAFE_CALL( hipMalloc( (void**)&v_ptr_func_dev       , sizeof(T)*total_size ) );
    HIP_SAFE_CALL( hipMalloc( (void**)&mat_mul_ptr_func_dev , sizeof(T)*total_size ) );
    ...
    HIP_SAFE_CALL( hipMemcpy( (void*) u_ptr_func_dev,   (void*)u_ptr_ok_host,      sizeof(T)*total_size, hipMemcpyHostToDevice ) );
    HIP_SAFE_CALL( hipMemcpy( (void*) v_ptr_func_dev,   (void*)v_ptr_ok_host,      sizeof(T)*total_size, hipMemcpyHostToDevice ) );
    ...
    HIP_SAFE_CALL( hipMemcpy( (void*) u_ptr_ok_dev,     (void*)u_ptr_ok_host,      sizeof(T)*total_size, hipMemcpyHostToDevice ) );
    HIP_SAFE_CALL( hipMemcpy( (void*) v_ptr_ok_dev,     (void*)v_ptr_ok_host,      sizeof(T)*total_size, hipMemcpyHostToDevice ) );
    ...
    if((tests == 'd')||(tests == 'a'))
    {
        ...
        {
            mat_mul_device_f<for_each_hip_t, array_device_t>(N, u_dev, v_dev, mat_mul_dev);
        }
        ...
        for(int it_ = 0; it_ < number_of_iters; it_++)
        {
            mat_mul_device_ptr<for_each_hip_t>(N, u_ptr_func_dev, v_ptr_func_dev, mat_mul_ptr_func_dev);
        }
        ...
        HIP_SAFE_CALL( hipMemcpy( (void*) mat_mul_ptr_func_check, (void*)mat_mul_ptr_func_dev, sizeof(T)*total_size, hipMemcpyDeviceToHost ) );
        std::cout << "gpu ptr_func diff        = " << check_coincide_ptr(N, mat_mul_ptr_ok_host,  mat_mul_ptr_func_check) << std::endl;


        HIP_SAFE_CALL( hipMemcpy( (void*) mat_mul_ptr_check, (void*)mat_mul_ptr_dev, sizeof(T)*total_size, hipMemcpyDeviceToHost ) );
#ifdef USE_CONST
        ...
        HIP_SAFE_CALL( hipMemcpy( (void*) mat_mul_ptr_ok_check, (void*)mat_mul_ptr_ok_dev, sizeof(T)*total_size, hipMemcpyDeviceToHost ) );
#ifdef USE_CONST
        ...
        std::string filename;
        filename = "mat_mul_hip_device_" + std::to_string(K) + "x" + std::to_string(K) + ".csv";
    ...
    if((tests == 'h')||(tests == 'a'))
    {
        ...
        std::string filename;
        filename = "mat_mul_hip_host_" + std::to_string(K) + "x" + std::to_string(K) +  ".csv";
    ...
    HIP_SAFE_CALL( hipFree(mat_mul_ptr_ok_dev) );
    HIP_SAFE_CALL( hipFree(v_ptr_ok_dev) );
    HIP_SAFE_CALL( hipFree(u_ptr_ok_dev) );

    HIP_SAFE_CALL( hipFree(mat_mul_ptr_dev) );
    HIP_SAFE_CALL( hipFree(v_ptr_dev) );
    HIP_SAFE_CALL( hipFree(u_ptr_dev) );

    HIP_SAFE_CALL( hipFree(mat_mul_ptr_func_dev) );
    HIP_SAFE_CALL( hipFree(v_ptr_func_dev) );
    HIP_SAFE_CALL( hipFree(u_ptr_func_dev) );
