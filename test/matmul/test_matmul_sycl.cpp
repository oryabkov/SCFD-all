...
struct sycl_selector
{
    ...
    bool               mem_set;
    sycl_selector() : mem_set( false )
    {
        cnt = 0;
    }
    sycl_selector( std::size_t free_mem ) : max_free_mem( free_mem ), mem_set( true )
    {
        cnt = 0;
    }
    ...
    int operator()( const sycl::device &device )
    {
        ...
        if ( !mem_set )
            std::cout << "cnt: " << cnt << "  Device: " << device.get_info<sycl::info::device::name>();
        ...
        if ( device_info == sycl::info::device_type::gpu ) //device.is_gpu() )//get_info<sycl::device::is_gpu()>() )
        {
            ...
            if ( !mem_set )
                std::cout << " is a gpu, total memory: " << total_mem_ << " bytes, free memory: " << free_mem_
                          << " bytes." << std::endl;
        }
        if ( device_info == sycl::info::device_type::cpu )
        {
            if ( !mem_set )
                std::cout << " is a cpu " << std::endl;
        }
        if ( device_info == sycl::info::device_type::accelerator )
        {
            if ( !mem_set )
                std::cout << " is an accelerator " << std::endl;
        }
...
#define __COMMON_PARTS_DEVICE_INIT__                                                                                   \
    std::cout << "Selected device: " << sycl_device_queue.get_device().get_info<sycl::info::device::name>()            \
              << std::endl;
