...
struct func_mat_mul
{
    ...
    func_mat_mul( const MatrixND &f1, const MatrixND &f2, MatrixND &f_out ) : f1_( f1 ), f2_( f2 ), f_out_( f_out )
    {
    }
...
struct func_mat_mul_mm
{
    ...
    func_mat_mul_mm( const MatrixND &f1, const MatrixND &f2, MatrixND &f_out ) : f1_( f1 ), f2_( f2 ), f_out_( f_out )
    {
    }
    __DEVICE_TAG__ void operator()( const int &idx ) const
    {
...
template <>
struct sycl::is_device_copyable<func_mat_mul_naive<T, array_device_t>> : std::true_type
{
};
template <>
struct sycl::is_device_copyable<func_mat_mul<T, array_device_t>> : std::true_type
{
};
template <>
struct sycl::is_device_copyable<func_mat_mul_mm<T, array_device_t>> : std::true_type
{
};
#endif
...
struct func_mat_mul_ptr_dev
{
    ...
    func_mat_mul_ptr_dev( std::size_t const sz, const T *f1, const T *f2, T *f_out )
        : N( sz ), f1_( f1 ), f2_( f2 ), f_out_( f_out )
    {
    }
...
#ifdef __COMMON_PARTS_USING_SYCL__
template <>
struct sycl::is_device_copyable<func_mat_mul_ptr_dev<T>> : std::true_type
{
};
template <>
struct sycl::is_device_copyable<func_mat_mul_ptr_host<T>> : std::true_type
{
};
#endif
...
T check_coincide_tensor( const std::size_t N, const T *ptr_, const MatrixND &ta_ )
{
    ...
    return std::isfinite( diff ) ? diff / N / K / K : std::numeric_limits<T>::infinity();
}
