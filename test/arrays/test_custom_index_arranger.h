...
int check()
{
    ...
    auto idx012 =
        ff<0>( 2, 3, 1 ) + ff<0>( Nx, Ny, Nz ) * ( ff<1>( 2, 3, 1 ) + ff<1>( Nx, Ny, Nz ) * ff<2>( 2, 3, 1 ) );
    std::cout << "012 (2,3,1): " << idx012 << ", correct:" << 2 + 3 * Nx + 1 * Nx * Ny << std::endl;

    auto idx201 =
        ff<2>( 2, 3, 1 ) + ff<2>( Nx, Ny, Nz ) * ( ff<0>( 2, 3, 1 ) + ff<0>( Nx, Ny, Nz ) * ff<1>( 2, 3, 1 ) );
    std::cout << "201 (2,3,1): " << idx201 << ", correct:" << 1 + 2 * Nz + 3 * Nz * Nx << std::endl;

    const int idx0    = 1;
    const int idx1    = 2;
    const int idx2    = 0;
    auto      idx_req = ff<idx0>( 2, 3, 1 ) +
                   ff<idx0>( Nx, Ny, Nz ) * ( ff<idx1>( 2, 3, 1 ) + ff<idx1>( Nx, Ny, Nz ) * ff<idx2>( 2, 3, 1 ) );
    ...
    std::cout << test2_01.calc_lin_index( 0, 0 ) << " " << test2_01.calc_lin_index( 10, 0 ) << " "
              << test2_01.calc_lin_index( 5, 5 ) << " " << test2_01.calc_lin_index( 0, 10 ) << std::endl;
    std::cout << test2_10.calc_lin_index( 0, 0 ) << " " << test2_10.calc_lin_index( 10, 0 ) << " "
              << test2_10.calc_lin_index( 5, 5 ) << " " << test2_10.calc_lin_index( 0, 10 ) << std::endl;

    std::cout << test3_012.calc_lin_index( 0, 0, 0 ) << " " << test3_012.calc_lin_index( 10, 0, 0 ) << " "
              << test3_012.calc_lin_index( 0, 9, 0 ) << " " << test3_012.calc_lin_index( 0, 0, 8 ) << " "
              << test3_012.calc_lin_index( 5, 5, 0 ) << " " << test3_012.calc_lin_index( 0, 5, 5 ) << " "
              << test3_012.calc_lin_index( 5, 0, 5 ) << " " << test3_012.calc_lin_index( 5, 5, 5 ) << std::endl;
    std::cout << test3_120.calc_lin_index( 0, 0, 0 ) << " " << test3_120.calc_lin_index( 10, 0, 0 ) << " "
              << test3_120.calc_lin_index( 0, 9, 0 ) << " " << test3_120.calc_lin_index( 0, 0, 8 ) << " "
              << test3_120.calc_lin_index( 5, 5, 0 ) << " " << test3_120.calc_lin_index( 0, 5, 5 ) << " "
              << test3_120.calc_lin_index( 5, 0, 5 ) << " " << test3_120.calc_lin_index( 5, 5, 5 ) << std::endl;
    std::cout << test3_201.calc_lin_index( 0, 0, 0 ) << " " << test3_201.calc_lin_index( 10, 0, 0 ) << " "
              << test3_201.calc_lin_index( 0, 9, 0 ) << " " << test3_201.calc_lin_index( 0, 0, 8 ) << " "
              << test3_201.calc_lin_index( 5, 5, 0 ) << " " << test3_201.calc_lin_index( 0, 5, 5 ) << " "
              << test3_201.calc_lin_index( 5, 0, 5 ) << " " << test3_201.calc_lin_index( 5, 5, 5 ) << std::endl;
    ...
    for ( int j = 0; j < Nx; j++ )
        for ( int k = 0; k < Ny; k++ )
            ...
            {
                check_diff(
                    { j, k, l }, vec_check3.at( ( Ny * j + k ) * Nz + l ), array012_view( j, k, l ), "012", ok_flag3
                );
                check_diff(
                    { j, k, l }, vec_check3.at( ( Ny * j + k ) * Nz + l ), array210_view( j, k, l ), "210", ok_flag3
                );
                check_diff(
                    { j, k, l }, vec_check3.at( ( Ny * j + k ) * Nz + l ), array120_view( j, k, l ), "120", ok_flag3
                );
                check_diff(
                    { j, k, l }, vec_check3.at( ( Ny * j + k ) * Nz + l ), array102_view( j, k, l ), "102", ok_flag3
                );
                check_diff(
                    { j, k, l }, vec_check3.at( ( Ny * j + k ) * Nz + l ), array201_view( j, k, l ), "201", ok_flag3
                );
                check_diff(
                    { j, k, l }, vec_check3.at( ( Ny * j + k ) * Nz + l ), array021_view( j, k, l ), "021", ok_flag3
                );
            }
    if ( ok_flag3 )
    {
