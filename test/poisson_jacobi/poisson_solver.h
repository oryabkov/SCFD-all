...
public:
    poisson_solver( idx_nd_type size, vec_type dom_sz )
        : mesh_( size, dom_sz ), rhs_( size ), x_( size ), x_residual_( size ), x_buf_( size ), x_ref_( size ),
          tmp_( size )
    {
    }
    ...
    bool solve( real eps, int max_iters )
    {
        ...
        for ( int i = 0; i < max_iters; ++i )
        {
            ...
            {
                std::cout << "poisson_solver::solve: target relative resudial norm " << eps
                          << " reached; stop iterations" << std::endl;
                return true;
            }
            std::swap( x_, x_buf_ );
        }
