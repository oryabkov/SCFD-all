...
#pragma unroll
            ...
            {
                rhs_val *=
                    st::sin( real( 2 ) * st::pi() * wave_numbers[j] * mesh.coord( j, idx[j] ) / mesh.dom_sz()[j] );
            }
            rhs( idx ) = rhs_val;
...
{
    for_each_nd_( [=] __DEVICE_TAG__( idx_nd_type idx ) { a( idx ) = real( 0 ); }, mesh_.size() );
}
...
T poisson_solver<T, ForEachND, Reduce, Memory, Dim>::perform_iter()
{
    ...
    for_each_nd_(
        [=] __DEVICE_TAG__( idx_nd_type idx ) {
            if ( mesh.check_is_on_border( idx ) )
                return;
